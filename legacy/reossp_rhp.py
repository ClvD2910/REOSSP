...
def generate_toy_instance(S=8, K=5, P=3, G=2, J_options=20, T_per_stage=500, seed=42):
    random.seed(seed)
    data = {}
    data["S"] = S
...
def solve_subproblem(data, s, L, J_tilde_prev, time_limit=None, verbose=False):
    ...
    # 5) at most one activity per time step
    for ell in Lset:
        for k in K_list:
            for t in T_s[ell]:
...
def solve(data, L=1, time_limit_per_sub=30, verbose=False):
    ...
    for s in range(1, S - L + 2):  # loop such that s..s+L-1 valid; last s = S-L+1
        if verbose:
            print(f"\n--- Solving RHP(s={s}, L={L}) ---")
        model, xvars, yvars, qvars, hvars, dvars, bvars = solve_subproblem(data, s, L, J_tilde_prev, time_limit=time_limit_per_sub, verbose=verbose)
...
if __name__ == "__main__":
    inst = generate_toy_instance(S=8, K=5, P=3, G=2, J_options=20, T_per_stage=200, seed=42)
    res = solve(inst, L=1, time_limit_per_sub=30, verbose=False)
    print("\n-- Summary --")
